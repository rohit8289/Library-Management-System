...
class Library {
    ...
    public void addBook(String title, String author) {
        ...
        try (FileWriter fw = new FileWriter("library.txt", true); // 'true' for append mode
                PrintWriter writer = new PrintWriter(fw)) {
            writer.println(title + "|" + author + "|" + book.isCheckedOut());
            System.out.println("Book added and saved to file successfully.");
        } catch (IOException e) {
            System.out.println("Error saving the new book to file: " + e.getMessage());
        }
    }
        ...
        for (Book book : books) {
            if (book.getTitle().equals(title)) {
                if (!book.isCheckedOut()) {
                    ...
                    saveBooksToFile("library.txt"); // Save changes to file
                } else {
    ...
    public void saveBooksToFile(String filename) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) { // Ensure to use FileWriter
            for (Book book : books) {
                writer.println(book.getTitle() + "|" + book.getAuthor() + "|" + book.isCheckedOut());
            }
            System.out.println("Library status updated in file: " + filename);
        } catch (IOException e) {
    ...
    public void loadBooksFromFile(String filename) {
        try (Scanner scanner = new Scanner(new File(filename))) {
            ...
            while (scanner.hasNextLine()) {
                ...
                // Validate that the line has exactly 3 parts
                if (parts.length == 3) {
...
public class LibraryManagementSystem {
    public static void main(String[] args) {
        ...
        // Load books from a file (if available)
